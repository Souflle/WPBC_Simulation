...
Epoch = 1000;       % the # of repeatation 
unknown_pp = 6;     % the # of state

MAX_harvest = zeros(Epoch, unknown_pp);
BS_harvest = zeros(Epoch, unknown_pp);
WET_harvest = zeros(Epoch, unknown_pp);
%% *System Parameter*
...
for e = 1:Epoch
for pp = 1:unknown_pp
distance = 1 * pp;      % What parameter wants to change
...
partition = [bin: bin: 6 * PL];   % Assume MF path gain normally smaller than 3.0
codebook = [partition, 8 * PL];   % unless, estimate it to 4.0
...
% 
% Total Noise: $N=\sqrt{\;\delta \;}h^b n+w\in C^{\mathrm{RXD}} \;$
% 
...
% 
% WET harvest power = $\eta \;\left(1-\delta \;\right)\cdot \left({|\left(h^f 
% \right)}^T \Phi {\;+\;n|}^2 \right)\cdot \;\left(2W\right)$                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           

TX_beamforming = mag_f_est / norm(mag_f_est) .* exp(1j * phase_f_est.');
bs_1 = conj(H_mmse(1, :)) ./ norm(H_mmse(1, :));


MAX_harvest(e, pp) = 2 * eff * (1-del) * (h_f' * h_f);  % Maximum Harvestable power corresponds to fading channel
BS_harvest(e, pp) = 2 * eff * (1 - del) * abs(bs_1 * h_f).^2;
WET_harvest(e, pp) = 2 * eff * (1-del) * abs(TX_beamforming' * h_f).^ 2;    % Proposed Scheme

%WET_harvest - BS_harvest;    
%MAX_harvest - WET_harvest;
...
figure
plt = [mean(MAX_harvest); mean(BS_harvest); mean(WET_harvest)] / 2;
plot(2:2:12, plt)
legend("MAX", "BS", "WET")